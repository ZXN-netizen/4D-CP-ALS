...
% Description: 
% Evaluates RMSE over varying SNR.
...
Rs_hat = (S_true.' * conj(S_true)) / K_snap; 

%% 3. Monte Carlo Simulation
RMSE_theta_prop = zeros(1, N_snr); RMSE_phi_prop = zeros(1, N_snr); RMSE_range_prop = zeros(1, N_snr);
CRLB_theta_snr = zeros(1, N_snr); CRLB_phi_snr = zeros(1, N_snr); CRLB_range_snr = zeros(1, N_snr);

disp('================================================================');
disp('Starting Monte Carlo Simulation & CRLB Evaluation (vs. SNR)...');
disp('================================================================');

for snr_idx = 1:N_snr
    noise_var_actual = signal_pwr_base / (10^(SNR_dB(snr_idx) / 10));
    
    % CRLB Computation
    FIM = K_snap * (2 / noise_var_actual) * real((D_total' * Pi_U_perp * D_total) .* kron(ones(3,3), Rs_hat.'));
    CRLB_mat = inv(FIM);
    CRLB_theta_snr(snr_idx) = sqrt(mean(diag(CRLB_mat(1:L, 1:L)))) * 180 / pi; 
    CRLB_phi_snr(snr_idx)   = sqrt(mean(diag(CRLB_mat(L+1:2*L, L+1:2*L)))) * 180 / pi;   
    CRLB_range_snr(snr_idx) = sqrt(mean(diag(CRLB_mat(2*L+1:3*L, 2*L+1:3*L))));            
    ...
    RMSE_theta_prop(snr_idx) = sqrt(sq_err_theta / (Q_MC * L));
    RMSE_phi_prop(snr_idx)   = sqrt(sq_err_phi / (Q_MC * L));
    RMSE_range_prop(snr_idx) = sqrt(sq_err_range / (Q_MC * L));
    
    fprintf('SNR = %3d dB | Theta: %6.4f (CRLB: %6.4f) | Phi: %6.4f (CRLB: %6.4f) | Range: %6.4f (CRLB: %6.4f)\n', ...
            SNR_dB(snr_idx), RMSE_theta_prop(snr_idx), CRLB_theta_snr(snr_idx), RMSE_phi_prop(snr_idx), CRLB_phi_snr(snr_idx), RMSE_range_prop(snr_idx), CRLB_range_snr(snr_idx));
end
...
save('CRLB_theta_snr.mat', 'CRLB_theta_snr');
save('CRLB_phi_snr.mat',   'CRLB_phi_snr'  );
save('CRLB_range_snr.mat', 'CRLB_range_snr');
